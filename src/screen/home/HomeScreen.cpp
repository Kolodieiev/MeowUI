...
HomeScreen::HomeScreen(GraphicsDriver &display) : IScreen{display}
{
    ...
    */
    ...
    NavBar *navbar = creator.getNavbar(ID_NAVBAR, STR_MENU, "", "");
...
void HomeScreen::update()
{
    ...
    {
        _input.reset(); // Перед переходом до іншого екрану, рекомендується очищувати ввід.
                        // Інакше може трапитися ситуація, коли необроблені натискання з поточного екрану, потраплять до наступного.
